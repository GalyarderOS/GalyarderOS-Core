  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      role: "ai",
      content:
        "👋 Hello! I am Galyarder, your AI Assistant. Ready to help with anything on your OS—just type your question or request!",
    },
  ]);
    ...
    <section className="w-full max-w-2xl mx-auto">
      <div className="rounded-2xl border border-primary/60 bg-card/70 px-4 py-6 shadow-lg flex flex-col gap-4 animate-fade-in min-h-[380px]">
        <div className="flex items-center gap-2 mb-1">
          ...
          <span className="font-semibold text-base">AI Assistant Chatbot</span>
        </div>
        <div
          id="ai-chatbot-scroll"
          className="flex-1 min-h-[180px] max-h-56 overflow-y-auto pb-2 space-y-3"
        ...
        >
              ...
              <div
                className={`max-w-[80%] px-3 py-2 rounded-xl text-sm ${
                  msg.role === "user"
                    ...
                    : "bg-muted text-slate-800 dark:text-slate-100"
                }`}
              >
                {msg.content}
              </div>
          ...
          {sending && (
            <div className="flex items-center mt-1 text-slate-500 gap-2 text-xs pl-1">
        ...
        <div className="flex gap-2 pt-2">
          <input
            ...
            className="flex-1 px-3 py-2 bg-muted/40 text-base rounded-xl border outline-none focus:ring-2 ring-primary/40 transition disabled:opacity-80"
          />
          <Button
            onClick={handleSend}
