...
const PersonalModulesSection = ({ onOpenAIAssistant, onOpenNotionAI }: PersonalModulesSectionProps) => {
  ...
  const handleModuleClick = (module: any, idx: number, e: React.MouseEvent) => {
    ...
    }, 150); // Little delay for ripple to show
  };
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 mb-16">
        {modules.map((module, index) => (
          <motion.div
            ...
            transition={{ delay: index * 0.09 + 0.4, type: "spring", stiffness: 120 }}
            whileHover={{
              scale: 1.045,
              boxShadow: "0px 14px 30px -3px rgba(80,120,220,0.14),0 0 32px 4px rgba(90,80,250,0.09)"
            }}
            whileTap={{ scale: 0.98 }}
            draggable
            ...
            className={`group cursor-pointer select-none relative transition-transform duration-300 will-change-transform 
                ${draggedIdx === index ? "scale-95 brightness-90 z-50" : ""}
              `}
            style={{
              transform: draggedIdx === index ? "rotate(1.5deg)" : undefined,
              transition: "box-shadow 0.23s cubic-bezier(0.4,0,0.2,1),transform 0.1s"
            }}
            onClick={e => handleModuleClick(module, index, e)}
          >
            <Card className="relative overflow-hidden border-2 border-border group-hover:border-primary transition-all duration-300 bg-card/80 hover:bg-card soft-shadow group-hover:soft-shadow-lg h-full">
              {/* Ripple */}
              <span
                ref={el => (rippleRefs.current[index] = el)}
                className="pointer-events-none absolute rounded-full bg-primary/20 opacity-60 scale-100 z-10"
                style={{ transform: "scale(0)", transition: "transform 0.35s" }}
              ></span>
              <CardHeader className="pb-4">
                <div
                  className={`w-full h-24 bg-gradient-to-br ${module.gradient} rounded-2xl flex items-center justify-center mb-4 group-hover:scale-105 transition-transform duration-500 border border-border`}
                  style={{
                    boxShadow: "0 0 16px 3px rgba(90,120,220,0.04)",
                  }}
                >
      ...
      {/* Ripple Animation global style */}
      <style>{`
        .animate-ripple-effect {
          animation: ripple-effect 0.55s cubic-bezier(0.4,0,0.2,1) forwards;
        }
        @keyframes ripple-effect {
          from { transform: scale(0); opacity: 0.95; }
          65%  { transform: scale(2.1); opacity: 0.62; }
