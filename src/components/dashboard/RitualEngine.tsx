        ...
        >
          <div className="grid md:grid-cols-2 gap-4">
            {habits.map((habit, index) => (
              <Card key={habit.id} className={`border-2 transition-all ${habit.completedToday ? 'border-green-200 bg-green-50 dark:bg-green-900/20' : 'border-slate-200 bg-slate-50 dark:bg-slate-700/50'}`}>
                <CardContent className="p-4">
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex-1">
                      <div className="flex items-center space-x-2 mb-2">
                        <h3 className="font-semibold text-slate-800 dark:text-slate-200">{habit.name}</h3>
                        <Badge className={getCategoryColor(habit.category)}>{habit.category}</Badge>
                      </div>
                      <p className="text-sm text-slate-600 dark:text-slate-400 mb-2">{habit.description}</p>
                    </div>
                    <Button
                      variant={habit.completedToday ? "default" : "outline"}
                      size="sm"
                      onClick={() => toggleHabit(habit.id)}
                      className={habit.completedToday ? "bg-green-600 hover:bg-green-700 text-white" : "border-2 hover:border-green-600"}
                    >
