...
const defaultVisionBoardItems: VisionBoardItem[] = [
    { id: "1", title: "Financial Freedom", imageUrl: "/placeholder.svg", priority: 1 },
    { id: "2", title: "Health & Wellness", imageUrl: "/placeholder.svg", priority: 2 },
];
...
const useVisionStore = create<VisionState>()(
  persist(
    (set, get) => ({
      visionStatement: { title: '', description: '' },
      goals: [],
      visionBoardItems: [],
    ...
    }),
    {
      ...
      onRehydrateStorage: () => (state, error) => {
        if (error) {
          console.error("Failed to rehydrate vision store", error);
          return;
        }
        if (state && state.goals.length === 0 && state.visionBoardItems.length === 0) {
          state.visionStatement = defaultVisionStatement;
          state.goals = defaultGoals;
          state.visionBoardItems = defaultVisionBoardItems;
        }
      },
    }
  )
